...
if biggest.size != 0:
    ...
    matrix = cv2.getPerspectiveTransform(pts1, pts2)  # GER
    imgInvWrapColored = img.copy()
    imgInvWrapColored = cv2.warpPerspective(imgSolvedDigits, matrix, (widthImg, heightImg))
    inv_perspective = cv2.addWeighted(imgInvWrapColored, 1, img, 0.5, 1)
    imgDetectedDigits = drawGrid(imgDetectedDigits)
    imgSolvedDigits = drawGrid(imgSolvedDigits)

    imageArray = ([img,imgThreshold,imgContours, imgBigContour],
                  [imgDetectedDigits, imgSolvedDigits,imgInvWrapColored,inv_perspective])
